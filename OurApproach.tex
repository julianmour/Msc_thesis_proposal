...
\begin{itemize}
    \item A local robustness verifier: Given a classifier and a layer-neighborhood $N(x)$ of an image $x$\Dana{previous section called it $I$, so pick a notation and fix accordingly}, it determines whether the classifier is robust in this neighborhood. 
...
In particular, the verifier only has to prove that one of the labels is the target object's label. %Since this is a multi-label classifier, we are interested only in the target class robustness;
    ...
    We adapt it to multi-label classifiers by \Dana{complete}.
    ...
    Instead, we assume a cutting weights vector $cw_x$\Dana{what is a cutting vector? you need to fiest provide a high level explanation before the math notation};
    ...
    \end{gather*}\Dana{what is the $\odot$ operation?}
